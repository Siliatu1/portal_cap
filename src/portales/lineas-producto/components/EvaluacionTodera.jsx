...
const EvaluacionTodera = ({ onBack, onSubmit, coordinadoraData }) => {
  ...
  const buscarInstructora = async (categoriaSeleccionada) => {
    ...
    try {
      ...
      const query = `filters[cap_instructoras][${categoriaSeleccionada}][$eq]=true&filters[nombre][$eq]=${pdvEncoded}&populate[cap_instructoras][filters][${categoriaSeleccionada}][$eq]=true`;
      const data = await getCapPdvs(query);
  ...
  const confirmarGuardado = async () => {
    ...
    try {
      const response = await createCapTodera(dataToSend);
