...
const FormularioPuntoVenta = ({ onBack, onSubmit, coordinadoraData }) => {
  ...
  useEffect(() => {
    const cargarFestivos = async () => {
      try {
        const festivos = await getPublicHolidays(2026, 'CO');
  ...
  useEffect(() => {
    const cargarFechasBloqueadas = async () => {
      try {
        const result = await getCapCafeFechas();
        if (result.data && Array.isArray(result.data)) {
  ...
  const handleBloquearFecha = async (fecha, estaBloqueada) => {
    ...
    try {
      if (estaBloqueada) {
        ...
        if (registro) {
          await deleteCapCafeFecha(registro.id);
  ...
  const confirmarGuardado = async () => {
    ...
    try {
      ...
      await createCapCafe(dataToSend);
      ...
      setMensaje({ texto: `Error al guardar la inscripción: ${error?.response?.message || error?.message || 'Error desconocido'}`, tipo: "error" });
    } finally {
      setLoading(false);
    }
  };
