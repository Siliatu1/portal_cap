...
import { getCapInstructoras, getCapPdvs, getHorariosInstructoras, updateHorarioInstructora } from '../../../services/apiService';
...
function VistaAdministrativa({ userData, onLogout }) {
    ...
    const cargarPuntosVenta = async () => {
      try {
        ...
        if (response.data?.length) {
          const puntos = response.data.map(pdv => ({
            id: pdv.id,
  ...
  useEffect(() => {
    ...
    const cargarDatos = async () => {
      try {
        ...
        const documentosInstructoras = instructorasData.map(inst => String(inst.attributes.documento || '').trim()).filter(Boolean);
        ...
        const responseHorarios = await getHorariosInstructoras(`filters[fecha][$gte]=${fechaInicioStr}&filters[fecha][$lte]=${fechaFinStr}&pagination[pageSize]=40000`);

        if (responseHorarios.data?.length) {
          // Procesar horarios y obtener lista única de instructoras
          let horariosConDatos = responseHorarios.data.map(item => {
          ...
          if (lineaSeleccionada !== 'todas') {
            horariosConDatos = horariosConDatos.filter(h => documentosInstructoras.includes(String(h.documento).trim()));
          }
          ...
          const instructorasConNombre = documentosUnicos.map((doc) => ({
            documento: doc,
            nombre: instructorasMap.get(String(doc).trim()) || `Instructora ${doc}`
          }));
      ...
      columns.push({
        ...
        children: [
          ...
          },
          {
            ...
            width: 100,
            onHeaderCell: () => ({ style: { background: '#5A8C6A', color: 'white', fontWeight: 600 } }),
            render: (text) => <span style={{ fontSize: 11, color: '#333' }}>{text || ''}</span>,
          },
        ],
      });
    });
  ...
  const handleDescargarExcel = () => {
    ...
    fechasSemana.forEach((_, idx) => {
      merges.push({ s: { r: 1, c: 2 + idx * 3 }, e: { r: 1, c: 2 + idx * 3 + 2 } });
    });
    ws['!merges'] = merges;

    const colWidths = [{ wch: 5 }, { wch: 32 }];
    fechasSemana.forEach(() => colWidths.push({ wch: 20 }, { wch: 22 }, { wch: 15 }));
    ws['!cols'] = colWidths;
  ...
  const handleDescargarPDF = () => {
    ...
    fechasSemana.forEach(fecha => {
      ...
      const mm = String(fecha.getMonth() + 1).padStart(2, '0');
      const yyyy = fecha.getFullYear();
      thDias += `<th colspan="3" style="background:#4A7C59;color:white;text-align:center;padding:6px;border:1px solid #ccc">${diaNombre} ${dd}/${mm}/${yyyy}</th>`;
    });
    ...
    fechasSemana.forEach(() => {
      thSub += `<th style="background:#5A8C6A;color:white;padding:5px;border:1px solid #ccc">PDV</th><th style="background:#5A8C6A;color:white;padding:5px;border:1px solid #ccc">Motivo</th><th style="background:#5A8C6A;color:white;padding:5px;border:1px solid #ccc">Horario</th>`;
    });
    ...
    datosSemanal.forEach((row, idx) => {
      ...
      fechasSemana.forEach(fecha => {
        const ts = fecha.getTime();
        const dayKey = `day_${ts}`;
        cells += `<td style="background:${bgColor};padding:5px;border:1px solid #ccc">${row[`${dayKey}_pdv`] || ''}</td><td style="background:${bgColor};padding:5px;border:1px solid #ccc">${row[`${dayKey}_motivo`] || ''}</td><td style="background:${bgColor};padding:5px;font-size:10px;border:1px solid #ccc">${row[`${dayKey}_horario_text`] || ''}</td>`;
      });
      tbody += `<tr>${cells}</tr>`;
    });
